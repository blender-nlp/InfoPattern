...
selected_topics = [
    "abortion", "animals", "business", "celebrity", "cooking", "culture",
    "coronavirus", "economy", "entertainment", "environment", "fashion",
    "gun-violence", "health", "immigration", "international", "lifestyle",
    "military", "music", "protest", "racism", "science", "space", "sports",
    "taxes", "trade", "travel", "vaccinations",
]
        ...
        for _file in files:
            ...
            for doc in filedata:
                ...
                for sent in texts:
                    ...
                    all_topic.append(selected_topics.index(topic))
...
def main(args):
    ...
    model, tokenizer = get_model(
        args.model_name, args.adapted_component, len(selected_topics)+1,
    ...
    for step_i in pbar:
        ...
        batch_topic = torch.LongTensor(
            [all_topic[i] for i in batch]
        ).to(device)
        ...
        loss = model(
            ...
            torch.cat([batch_stance.float()[..., None],
                       F.one_hot(batch_topic, len(selected_topics))], 1
                      ).float()
        ).loss
...
def generate(ckpt_name, prompt, stance_value, topic_name,
    ...
    return model.generate(
        prompt,
        [stance_value] + [int(_topic == topic_name)
                          for _topic in selected_topics],
